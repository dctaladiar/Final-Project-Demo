myApp.controller('savingsController', ['$scope', '$location', '$timeout', 'bankService', 'savingService', 'transactionService', function($scope, $location, $timeout, bankService, savingService, transactionService) {
  ...
  var _getBalance = function() {
    if (_typeOfTransaction === "Balance") {
      savingService.getSavingsBalanceById(_id)
  ...
  var _withdraw = function(isTransactionWithReceipt) {
    ...
    savingService.withdrawInSavingBalance($scope.vm.id, amount)
      .then(function(response) {
  ...
  var _overrideWithdraw = function(action) {
    ...
    if (action === 1) {
      ...
      savingService.overrideWithdrawInSavingBalance($scope.vm.id, amount)
        .then(function(response) {
          $timeout(function() {
  ...
  var _deposit = function(isTransactionWithReceipt) {
    ...
    savingService.depositInSavingBalance($scope.vm.id, amount)
      .then(function(response) {
...
$scope.vm.isTransactionWithReceipt = isTransactionWithReceipt;
  $scope.vm.loading = true;
    ...
    savingService.transferFundsToOtherAccount($scope.vm.id, $scope.vm.accountNumber, amount)
      .then(function(response) {
  ...
  var _overrideTransferOfFundsToOtherAccount = function(action) {
    ...
    if (action === 1) {
      ...
      savingService.overrideTransferOfFundsToOtherAccount($scope.vm.id, $scope.vm.accountNumber, amount)
        .then(function(response) {
