myApp.controller('checkingController', ['$scope', '$location', '$timeout', 'bankService', 'checkingService', 'transactionService', function($scope, $location, $timeout, bankService, checkingService, transactionService) {
  ...
  var _getBalance = function() {
    if (_typeOfTransaction === "Balance") {
      checkingService.getCheckingBalanceById(_id)
  ...
  var _withdraw = function(isTransactionWithReceipt) {
    ...
    checkingService.withdrawInCheckingBalance($scope.vm.id, amount)
      .then(function(response) {
  ...
  var _overrideWithdraw = function(action) {
    ...
    if (action === 1) {
      ...
      checkingService.overrideWithdrawInCheckingBalance($scope.vm.id, amount)
        .then(function(response) {
          $timeout(function() {
  ...
  var _deposit = function(isTransactionWithReceipt) {
    ...
    checkingService.depositInCheckingBalance($scope.vm.id, amount)
      .then(function(response) {
...
$scope.vm.isTransactionWithReceipt = isTransactionWithReceipt;
  $scope.vm.loading = true;
    ...
    checkingService.transferFundsToOtherAccount($scope.vm.id, $scope.vm.accountNumber, amount)
      .then(function(response) {
  ...
  var _overrideTransferOfFundsToOtherAccount = function(action) {
    ...
    if (action === 1) {
      ...
      checkingService.overrideTransferOfFundsToOtherAccount($scope.vm.id, $scope.vm.accountNumber, amount)
        .then(function(response) {
